<template>
  <div>
    <detail-header title="新增专业队" :breadcrumbList="breadcrumbList"></detail-header>
    <div
      class="fromHeader"
      @click="
        () => {
      ...
      <el-row type="flex">
        <el-col :span="6" style="margin-right: 40px">
          <div class="grid-content bg-purple">
            ...
            <div class="formBox">
              <el-select v-model="responsiblePersonId" multiple placeholder="请选择联络员" @change="changePerson">
      ...
      <el-row type="flex">
        ...
        <el-col :span="6" style="margin-right: 40px">
          <div class="grid-content bg-purple">
            ...
            <div class="formBox">
              <el-input v-model="addform.longitude" placeholder="请输入经度" oninput="value=value.replace(/[^\d\.]/g,'')" class="formInp" type="text"></el-input>
            </div>
          </div>
        </el-col>
        <el-col :span="6" style="margin-right: 40px">
          <div class="grid-content bg-purple">
            ...
            <div class="formBox">
              <el-input v-model="addform.latitude" placeholder="请输入纬度" oninput="value=value.replace(/[^\d\.]/g,'')" class="formInp" type="text"></el-input>
            </div>
          </div>
        </el-col>
        <!-- <el-col :span="6" style="margin-right:40px" :offset="3">
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbList: ["首页", "系统管理", "专业队管理", "新增专业队"],
  ...
  mounted() {
    ...
    if (this.$store.state.roleIdList.includes(2)&&this.$store.state.roleIdList.length == 1) {
      this.addform.administrativeCode = this.$store.state.longData.administrativeCode;
    }
  },
  methods: {
    getliaisonMan(Id) {
    ...
    confirm() {
      ...
      this.isloading = true;
      this.$api.teamadd(data).then((res) => {
    ...
    changePerson(value){
     console.log(value);
     this.addform.liaisonUserIdStr = value?.join(',')
      // this.addform.responsiblePersonId = value
      // this.addform.responsiblePerson = this.userliaisonLisr.find(item=>item.id = value).name
    }
  },
};
</script>
