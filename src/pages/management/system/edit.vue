...
export default {
  ...
  data() {
    return {
      ...
      teamname: "",
      time: "",
      breadcrumbList: ["首页", "系统管理", "角色管理", "编辑"],
      value: "",
      options: [],
      defaultkes: [],
      controLdefaultkes: [],
  ...
  methods: {
        ...
        if (res.code == 200) {
          ...
          this.jurisdictionList.forEach((item) => {
            ...
            if (item.possess) {
              ...
              checkandHalfList.push(item.permissionsId);
            }
            // 获取拥有二级权限的二级Id
            if (item.secondList.length) {
              item.secondList.forEach((item2, index) => {
                if (item2.possess) {
                  if (flag) {
                    flag = false;
                    // 取一级用于权限入参
                  ...
                  checkandHalfList.push(item2.permissionsId);
          ...
          this.getCheckedKeys(checkandHalfList);
        } else {
    ...
    getCheckedKeys(keys = []) {
      let handList = [];
      let checkedIdList = [];
      // 详情传参此时权限列表未渲染不能使用方法获取节点
      if (keys.length) {
        checkedIdList = keys;
      } else {
        // 点击确认无需传参直接调用方法获取节点
        checkedIdList = this.$refs.tree
          .getCheckedKeys()
    ...
    confirm() {
      ...
      this.$api.roleupdate(this.addform).then((res) => {
