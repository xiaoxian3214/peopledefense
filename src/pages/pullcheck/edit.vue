  ...
  methods: {
      ...
      this.$api
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            // 每次更换左侧列表数据源,判断右边的数据是否在左边存在,如果不存在对象push到左边数据一份,用于回显数据
      ...
      this.$api.pullcheckdetail({ id: this.id }).then((res) => {
        if (res.code == 200) {
          ...
          this.personnelListValue =
            this.addform.professionalTeamPersonInformationList.map(
              (item) => item.id
            );
        } else {
          this.$message.error(res.message);
    ...
    confirm() {
      ...
      data.professionalTeamPersonInformationList = JSON.stringify(
        data.professionalTeamPersonInformationList
      );
      data.equipmentList = JSON.stringify(data.equipmentList);
      this.isloading = true;
      this.$api.pullcheckupdate(data).then((res) => {
